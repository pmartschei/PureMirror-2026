...
namespace Runtime
{
    using GetOverlayAPI_t = OverlayAPI* (*)();
    static HINSTANCE g_overlayDLL = NULL;
    static std::vector<OverlayAPI*> g_OverlayAPIs = {};
    ...
    static LRESULT WINAPI WndProc(const HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        ...
        for (auto& overlayApi : g_OverlayAPIs)
        {
            auto result = overlayApi->HandleInput(hWnd, uMsg, wParam, lParam);
            if (result != 0)
                return result;
        }
    ...
    void InitializeLibs()
    {
        g_overlayDLL = LoadLibraryA("PureMirror.Overlay.dll");
        if (g_overlayDLL)
        {
            auto GetOverlayAPI = reinterpret_cast<GetOverlayAPI_t>(GetProcAddress(g_overlayDLL, "GetOverlayAPI"));

            if (GetOverlayAPI)
            {
                auto overlayAPI = GetOverlayAPI();
                // TODO Version check imgui check whatever
                g_OverlayAPIs.push_back(overlayAPI);

                overlayAPI->Initialize(&g_rendererAPI);
            }
        }
    }
    ...
    void Render(RenderContext& renderContext)
    {
        for (auto& overlayApi : g_OverlayAPIs)
        {
            overlayApi->Render(&renderContext);
        }
    }
