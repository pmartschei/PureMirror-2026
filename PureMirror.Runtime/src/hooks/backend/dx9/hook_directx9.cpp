...
static bool CreateDeviceD3D9(HWND hWnd)
{
    ...
    HRESULT hr = g_pD3D->CreateDevice(
        D3DADAPTER_DEFAULT, D3DDEVTYPE_NULLREF, hWnd, D3DCREATE_SOFTWARE_VERTEXPROCESSING, &d3dpp, &g_pd3dDevice);
    if (hr != D3D_OK)
    {
...
static std::add_pointer_t<HRESULT WINAPI(IDirect3DDevice9*, const RECT*, const RECT*, HWND, const RGNDATA*, DWORD)>
    oPresentEx;
...
namespace DX9
{
    void Hook(HWND hwnd)
    {
        ...
        if (g_pd3dDevice)
        {
            ...
            static MH_STATUS resetStatus =
                MH_CreateHook(reinterpret_cast<void**>(fnReset), &hkReset, reinterpret_cast<void**>(&oReset));
            static MH_STATUS resetExStatus =
                MH_CreateHook(reinterpret_cast<void**>(fnResetEx), &hkResetEx, reinterpret_cast<void**>(&oResetEx));

            static MH_STATUS presentStatus =
                MH_CreateHook(reinterpret_cast<void**>(fnPresent), &hkPresent, reinterpret_cast<void**>(&oPresent));
            static MH_STATUS presentExStatus = MH_CreateHook(
                reinterpret_cast<void**>(fnPresentEx), &hkPresentEx, reinterpret_cast<void**>(&oPresentEx));
...
namespace DX9
{
    void Hook(HWND hwnd)
    {
        LOG("[!] DirectX9 backend is not enabled!\n");
    }
    void Unhook() {}
}  // namespace DX9
#endif
